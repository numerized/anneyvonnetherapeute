...
const markEmailTemplatesComplete = async () => {
  console.log(`Marking "Email Template" checklist items as complete for ${TASK_IDS.length} tasks...`);
  ...
  for (const taskId of TASK_IDS) {
    ...
    for (const checklist of checklists) {
      ...
      const emailTemplateItem = checklist.items.find(item => item.name === "Email Template");
      if (!emailTemplateItem) {
        console.log(`No "Email Template" item found in checklist ${checklist.id} for task ${taskId}`);
        continue;
      }
      
      // Skip if already resolved
      if (emailTemplateItem.resolved) {
        console.log(`"Email Template" item already marked as complete in task ${taskId}`);
        successCount++;
        continue;
      }
      
      const success = await markChecklistItemComplete(taskId, checklist.id, emailTemplateItem.id);
      if (success) {
        successCount++;
  ...
  console.log(`✅ Successfully marked "Email Template" as complete for ${successCount} out of ${TASK_IDS.length} tasks`);
};
