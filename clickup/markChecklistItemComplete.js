...
const itemNameToMark = args[0] || "Email Template"; // Default to "Email Template" if no argument provided
const taskIds = args.slice(1); // Any additional arguments are treated as task IDs

// If no task IDs provided, use these default IDs from our most recent run
...
// Use provided task IDs or default to all tasks
const TASK_IDS = taskIds.length > 0 ? taskIds : DEFAULT_TASK_IDS;
...
const getTaskDetails = async (taskId) => {
  try {
    ...
    if (response.data && response.data.checklists) {
      console.log(`✅ Found ${response.data.checklists.length} checklists for task ${taskId}`);
    ...
    console.error(`❌ Error getting task details for ${taskId}:`, error.response?.data || error.message);
    return null;
  }
};
...
const markChecklistItemComplete = async (taskId, checklistId, itemId) => {
  try {
    ...
    try {
      ...
      if (response.status === 200) {
        console.log(`✅ Checklist item ${itemId} marked as complete (alternative method)`);
        return true;
      } else {
        console.error(`❌ Failed to mark checklist item ${itemId} as complete (alternative method)`);
        return false;
      }
    } catch (altError) {
      console.error(`❌ Error marking checklist item ${itemId} as complete (both methods):`, 
...
// Main function
const markChecklistItems = async () => {
  console.log(`Marking "${itemNameToMark}" checklist items as complete for ${TASK_IDS.length} tasks...`);
  ...
  for (const taskId of TASK_IDS) {
    ...
    for (const checklist of checklists) {
      // Find the target checklist item
      const targetItem = checklist.items.find(item => item.name === itemNameToMark);
      if (!targetItem) {
        console.log(`No "${itemNameToMark}" item found in checklist ${checklist.id} for task ${taskId}`);
        continue;
      }
      
      // Skip if already resolved
      if (targetItem.resolved) {
        console.log(`"${itemNameToMark}" item already marked as complete in task ${taskId}`);
        successCount++;
        continue;
      }
      
      const success = await markChecklistItemComplete(taskId, checklist.id, targetItem.id);
      if (success) {
        successCount++;
        break; // Break after finding and marking the first matching item
      }
    }
  }
  
  console.log(`✅ Successfully marked "${itemNameToMark}" as complete for ${successCount} out of ${TASK_IDS.length} tasks`);
};
