...
const markChecklistItems = async () => {
  console.log(
    `Marking "${itemNameToMark}" checklist items as complete for ${TASK_IDS.length} tasks...`,
  )
  ...
  for (const taskId of TASK_IDS) {
    ...
    for (const checklist of checklists) {
      ...
      const targetItem = checklist.items.find(
        (item) => item.name === itemNameToMark,
      )
      if (!targetItem) {
        console.log(
          `No "${itemNameToMark}" item found in checklist ${checklist.id} for task ${taskId}`,
        )
        continue
      }
      ...
      if (targetItem.resolved) {
        console.log(
          `"${itemNameToMark}" item already marked as complete in task ${taskId}`,
        )
        successCount++
        continue
      }
  ...
  console.log(
    `✅ Successfully marked "${itemNameToMark}" as complete for ${successCount} out of ${TASK_IDS.length} tasks`,
  )
}
