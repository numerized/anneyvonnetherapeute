...
const getWorkspaceUsers = async () => {
  try {
    ...
    console.error("❌ Error getting workspace users:", error.response?.data || error.message);
    return [];
  }
};
...
const updateWorkspaceSettings = async () => {
  try {
    console.log("Updating workspace settings to show completed checklist items for all users...");
    ...
    if (response.status === 200) {
      console.log("✅ Successfully updated workspace settings to show completed checklist items for all users");
      return true;
    } else {
    ...
    console.error("❌ Error updating workspace settings:", error.response?.data || error.message);
    return false;
  }
};
...
const updateUserPreferences = async (user) => {
  try {
    console.log(`Updating preferences for user: ${user.user.username} (ID: ${user.user.id})...`);
    ...
    if (response.status === 200) {
      console.log(`✅ Successfully updated preferences for user: ${user.user.username}`);
      return true;
    } else {
      console.error(`❌ Failed to update preferences for user: ${user.user.username}`);
      return false;
    }
  } catch (error) {
    console.error(`❌ Error updating preferences for user ${user.user.username}:`, error.response?.data || error.message);
    return false;
  }
};
...
// Main function
const main = async () => {
  console.log("Starting to update preferences to show completed checklist items...");
  ...
  if (workspaceSuccess) {
    ...
    console.log("   All users should refresh their ClickUp interface to see the changes");
    return;
  }
