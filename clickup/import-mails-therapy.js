    ...
    try {
      ...
      if (taskResponse.data.id) {
        ...
        try {
          ...
          console.log(`  Attempting to create checklist for task ID: ${taskId}...`);
          ...
          if (checklistResponse.data && checklistResponse.data.checklist && checklistResponse.data.checklist.id) {
            ...
            console.log(`  ✅ Checklist created for ${task.name} (ID: ${checklistId})`);
            ...
            for (const item of task.checklist) {
              try {
                ...
                console.error(`    ❌ Error adding checklist item: ${item}`, itemError.response?.data || itemError.message);
              }
            }
          } else {
          ...
          console.error(`  ❌ Error creating checklist for ${task.name}:`, checklistError.response?.data || checklistError.message);
        }
      }
    } catch (error) {
      console.error(`❌ Error creating task: ${task.name}`, error.response?.data || error.message);
    }
  }
};
