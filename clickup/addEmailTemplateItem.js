...
const addEmailTemplateItem = async (taskId) => {
  try {
    ...
    console.log(`Adding "Email Template" to task ${taskId} (checklist: ${checklistId})...`);
    ...
    console.error(`❌ Error adding "Email Template" to task ${taskId}:`, error.response?.data || error.message);
    return false;
  }
};
...
const addEmailTemplateToAllTasks = async () => {
  console.log(`Adding "Email Template" checklist item to ${TASK_IDS.length} tasks...`);
  ...
  console.log(`✅ Successfully added "Email Template" to ${successCount} out of ${TASK_IDS.length} tasks`);
};
