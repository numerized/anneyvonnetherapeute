...
const CAPSULES_FILE_PATH = path.join(__dirname, '../components/pages/espace180/data/capsules.ts');
...
async function updateCapsulesWithDuration() {
  try {
    ...
    const capsulesRegex = /export\s+const\s+capsules\s*:\s*Capsule\[\]\s*=\s*\[([\s\S]*?)\]/;
    ...
    const updatedCapsuleObjects = capsuleObjects.map(capsuleStr => {
      ...
      if (hasDuration) {
        ...
        return capsuleStr.replace(/duration\s*:\s*['"][^'"]+['"]/, `duration: "${formattedDuration}"`);
      } else {
        // Add duration property before the last property (which is typically mediaType)
        return capsuleStr.replace(/(\s*)(mediaType\s*:)/, `$1duration: "${formattedDuration}",$1$2`);
      }
    });
...
if (!checkFfprobeAvailability()) {
  console.error('Error: ffprobe is not installed or not in PATH. Please install ffmpeg/ffprobe first.');
  process.exit(1);
}
