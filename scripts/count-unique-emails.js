  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      if (!firstRegistrationDate) {
        ...
        console.log(
          `Date du premier abonnement: ${firstRegistrationDate.toISOString().split('T')[0]}`,
        )
      }

      // Calculate which week this registration belongs to (0-indexed)
      const weeksSinceStart = Math.floor(
        (timestamp - firstRegistrationDate) / (7 * 24 * 60 * 60 * 1000),
      )
      ...
      if (!weekLabels[weekNum]) {
        ...
        weekLabels[weekNum] =
          `Semaine ${weekNum} (${dates.start} au ${dates.end})`
      }
    ...
    sortedWeeks.forEach((week) => {
      ...
      totalEmails += count
      console.log(
        `${week}: ${count} nouveaux emails uniques (Total: ${totalEmails})`,
      )
    })
    ...
    console.error(
      'Erreur lors de la récupération des abonnés newsletter:',
      error,
    )
  }
}
