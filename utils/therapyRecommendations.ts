...
export const getMatchingOfferingsOptions = (keywords: string[], situation: string = '', maxResults: number = 2): TherapyOption[] => {
  const allOfferings = [...prepareTherapyOfferings(), ...prepareCoachingOfferings()]
  
  // Filter offerings by keywords
  let filteredOfferings = allOfferings.filter((offering) => {
    // For specific situations, filter by type
    if (situation === 'couple' && !coupleSpecificOfferings.includes(offering.type)) {
      return false
    } else if (situation === 'individual' && coupleSpecificOfferings.includes(offering.type)) {
      return false
    }
    
    // Simple keyword matching in title and description
    const offeringText = `${offering.title} ${offering.description}`.toLowerCase()
    return keywords.some((keyword) => offeringText.includes(keyword.toLowerCase()))
  })
  ...
  filteredOfferings.sort((a, b) => {
    ...
    const aMatches = keywords.filter((keyword) => aText.includes(keyword.toLowerCase())).length
    const bMatches = keywords.filter((keyword) => bText.includes(keyword.toLowerCase())).length
...
export const generateRecommendedOptions = (situation: string, priority: string, challenge: string): TherapyOption[] => {
  ...
  const findOffering = (type: string): TherapyOption | undefined => {
    return allOfferings.find((offering) => offering.type === type || offering.therapyId === type)
  }
  ...
  const situationKeyword = situation === 'A' ? 'célibataire' : 
                         situation === 'B' ? 'couple' : 
                         situation === 'C' ? 'questionnement' : 'rupture'
  ...
  switch(priority) {
    ...
    default:
      ...
      recommendedOptions = getMatchingOfferingsOptions(keywords, 
  ...
  if (recommendedOptions.length < 2) {
    ...
    const additionalRecommendations = filteredOfferings
      .filter(o => !recommendedOptions.some(r => r.therapyId === o.therapyId))
  ...
  if (vitOffering && !recommendedOptions.some(r => r.therapyId.includes('vit'))) {
...
export const getIntentionText = (priority: string, challenge: string): string => {
  if (priority === 'A1') {
