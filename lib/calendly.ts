 ...
 */
export async function getCalendlyEventDetails(
  eventUri: string,
): Promise<CalendlyEventDetails | null> {
  try {
    ...
    const capitalizedStartTime =
      formattedStartTime.charAt(0).toUpperCase() + formattedStartTime.slice(1)
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        console.error(
          'Authentication error: The Calendly API key is likely invalid or expired',
        )
        console.error(
          'Please check that the API key is correctly set in the environment variables',
        )
        console.error(
          'For V2 API, you need to use a Personal Access Token, not the older API key',
        )
      } else if (error.response.status === 404) {
        console.error(
          'Event not found: The Calendly event ID may be invalid or the event has been deleted',
        )
      }
    } else if (error.request) {
      // Request was made but no response was received
      console.error(
        'Error fetching Calendly event details: No response received from server',
      )
      console.error(error.request)
    } else {
      // Something happened in setting up the request that triggered an Error
      console.error(
        'Error setting up request to fetch Calendly event details:',
        error.message,
      )
    }
    ...
    if (error.config) {
      console.error('Error configuration:', {
        ...
        headers: error.config.headers
          ? 'Headers present (not shown for security)'
          : 'No headers',
      })
    }
