...
export interface SessionDetails {
  ...
  location?:
    | {
        // Session location (V2 API format)
...
export async function migrateUserToEnglishProperties(
  userId: string,
): Promise<void> {
  const db = getFirestore(app)
  const userRef = doc(db, 'users', userId)
...
export async function migrateAllUsersToEnglish(): Promise<void> {
  ...
  const migrations = usersSnapshot.docs.map((doc) =>
    migrateUserToEnglishProperties(doc.id),
  )
