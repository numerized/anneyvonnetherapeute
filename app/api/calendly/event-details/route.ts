...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (process.env.CALENDLY_API_KEY) {
      console.log('[Calendly API] Using V2 API authentication format (Bearer token)');
      headers['Authorization'] = `Bearer ${apiKey}`;
    } 
    // For V1 API or fallback, use the older token format
    else {
      console.log('[Calendly API] Using V1 API authentication format or fallback');
    ...
    if (response.data && response.data.resource && response.data.resource.uri) {
      ...
      console.log(`[Calendly API] Successfully fetched event: ${eventResource.name || 'Unknown event'}`);
      ...
      try {
        ...
        console.error('[Calendly API] Error fetching invitee details:', inviteeError);
      }
    }
    ...
    console.error('[Calendly API] Error details:', error.response?.data || 'No detailed error information');
