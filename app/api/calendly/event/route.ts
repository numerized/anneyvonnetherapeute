...
export async function GET(request: Request) {
  ...
  try {
    ...
    const apiKey = process.env.CALENDLY_API_KEY || process.env.CALENDLY_CLIENT_SECRET || process.env.NEXT_PUBLIC_CALENDLY_API_KEY;
    ...
    console.log('[Calendly API V2] Using key from:', 
      process.env.CALENDLY_API_KEY ? 'CALENDLY_API_KEY' : 
      (process.env.CALENDLY_CLIENT_SECRET ? 'CALENDLY_CLIENT_SECRET' :
      (process.env.NEXT_PUBLIC_CALENDLY_API_KEY ? 'NEXT_PUBLIC_CALENDLY_API_KEY' : 'No key found')));
    ...
    if (process.env.CALENDLY_API_KEY) {
      console.log('[Calendly API V2] Using V2 API authentication format (Bearer token)');
      headers['Authorization'] = `Bearer ${apiKey}`;
    } 
    // For V1 API or fallback, use the older token format
    else {
      console.log('[Calendly API V2] Using V1 API authentication format or fallback');
    ...
    if (response.data && response.data.resource && response.data.resource.uri) {
      ...
      console.log(`[Calendly API V2] Successfully fetched event: ${eventResource.name || 'Unknown event'}`);
      ...
      try {
        ...
        console.error('[Calendly API V2] Error fetching invitee details:', inviteeError);
    ...
    console.error('[Calendly API V2] Error fetching Calendly event:', error.response?.data || error.message);
    
    // Log more details about the error
    if (error.response) {
      console.error(`[Calendly API V2] Status: ${error.response.status}, Data:`, error.response.data);
      
      // Special handling for 401 unauthorized errors
      if (error.response.status === 401) {
        console.error('[Calendly API V2] 401 Unauthorized - API key is likely invalid or expired');
        console.error('For V2 API, make sure you are using a Personal Access Token, not the older API key');
        
        // Provide detailed instructions for fixing the authentication issue
        return NextResponse.json(
          { 
            error: 'Authentication failed with Calendly API V2',
            details: 'The Calendly API token may be invalid, expired, or missing. For V2 API, you need to use a Personal Access Token.',
            instructions: `
...
`,
            ...
            debug: {
              envVars: {
                ...
                hasPublicCalendlyApiKey: !!process.env.NEXT_PUBLIC_CALENDLY_API_KEY
              }
            }
          },
          { status: 401 }
    ...
    const errorMessage = error.response?.data?.message || 'Failed to fetch Calendly event details';
