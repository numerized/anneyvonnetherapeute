...
const TherapyTimeline: FC<TherapyTimelineProps> = ({ events }) => {
  ...
  const [selectedEmail, setSelectedEmail] = useState<EmailModalState | null>(
    null,
  )
  ...
  const getEmailType = (
    event: TherapyTimelineEvent,
  ): TherapyEmailType | null => {
    ...
    if (event.title.includes('Préparation Séance 1')) {
      return event.title.includes('Homme')
      ...
      return event.title.includes('Homme')
        ? TherapyEmailType.AFTER_INDIV_3
        : TherapyEmailType.AFTER_INDIV_3
  ...
  const isFemaleEvent = (event: TherapyTimelineEvent): boolean => {
    return (
      event.title.toUpperCase().includes('FEMME') ||
      event.title.includes('- Femme')
    )
  }

  const isMaleEvent = (event: TherapyTimelineEvent): boolean => {
    return (
      event.title.toUpperCase().includes('HOMME') ||
      event.title.includes('- Homme')
    )
  }
  ...
  const renderTimelineEvent = (event: TherapyTimelineEvent, index: number) => {
    ...
    return (
      <div
        key={`timeline-${event.title}-${index}`}
        className={`relative ${isWomanEvent ? 'pl-12' : 'pl-12'}`}
      >
        {/* Icon */}
        <div
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {event.partner && (
              <span
                style={{
                  backgroundColor:
                    event.partner === 'male' ? '#DBEAFE' : '#FCE7F3',
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handlePartnerInfoChange('homme', 'firstName', e.target.value)
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handlePartnerInfoChange('homme', 'lastName', e.target.value)
                }
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handlePartnerInfoChange('femme', 'firstName', e.target.value)
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handlePartnerInfoChange('femme', 'lastName', e.target.value)
                }
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-pink-500 focus:outline-none focus:ring-1 focus:ring-pink-500"
              />
            </div>
          ...
          {events.map((event, index) => {
            ...
            if (!isWomanEvent && !isManEvent) {
              return (
                <div
                  key={`event-${index}`}
                  className="col-span-2 mx-auto w-2/3"
                >
                ...
                (e) =>
                  e.title.replace(' - Homme', '').replace(' - Femme', '') ===
                    event.title
                      .replace(' - Homme', '')
                      .replace(' - Femme', '') &&
                  ((isManEvent && isFemaleEvent(e)) ||
                    (isWomanEvent && isMaleEvent(e))),
              )
              ...
              if (isManEvent) {
                return (
                  ...
                  >
                    ...
                    <div className="relative">
                      {matchingEvent &&
                        renderTimelineEvent(matchingEvent, index)}
                    </div>
                  </div>
                )
              }
