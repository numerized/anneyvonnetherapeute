...
export const CommonBenefits: React.FC<CommonBenefitsProps> = ({ benefits }) => {
  ...
  const getBenefitIcon = (benefit: string) => {
    if (
      benefit.toLowerCase().includes('sms') ||
      benefit.toLowerCase().includes('whatsapp')
    ) {
      return <MessageCircle size={24} />
    } else if (
      benefit.toLowerCase().includes('plateforme') ||
      benefit.toLowerCase().includes('accès')
    ) {
      return <BookOpen size={24} />
    } else if (
      benefit.toLowerCase().includes('délocalisation') ||
      benefit.toLowerCase().includes('partout')
    ) {
      return <Globe size={24} />
    } else if (
      benefit.toLowerCase().includes('capsule') ||
      benefit.toLowerCase().includes('audio')
    ) {
      ...
      <div className="bg-primary-dark/30 backdrop-blur-sm rounded-[24px] p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {benefits.map((benefit, idx) => (
            <div
              key={idx}
              className="flex items-start gap-4 bg-primary-forest/30 p-6 rounded-[16px]"
            >
