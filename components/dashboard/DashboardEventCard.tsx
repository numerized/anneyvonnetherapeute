...
export function DashboardEventCard({ event, sessionDetails }: DashboardEventCardProps) {
  ...
  if (!sessionDetails || !sessionDetails.calendarEvent) {
    return (
      <div className="bg-primary-cream/10 p-4 rounded-lg shadow-sm">
        <h3 className="font-medium text-[rgb(247_237_226_)]">
          {event.title}
        </h3>
...
export function DashboardSessionInfo({ event, sessionDetails }: DashboardEventCardProps) {
  if (!sessionDetails) {
    return (
  ...
  const displayDate = sessionDetails.formattedDateTime || 
    (sessionDetails.date ? new Date(sessionDetails.date).toLocaleDateString('fr-FR') : 'Date non disponible');

  return (
    <div className="text-sm">
      <div className="flex items-center justify-between">
        <div className="font-medium text-[rgb(247_237_226_)]">{event.title}</div>
      ...
      <div className="text-[rgb(247_237_226_)]/60 mt-1">
        {displayDate}
      </div>
    </div>
  );
