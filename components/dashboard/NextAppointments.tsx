...
export function NextAppointments({ users }: NextAppointmentsProps) {
  ...
  const isCoupleSession = (type: string) => {
    return (
      type.startsWith('partner1_session_') ||
      type.startsWith('partner2_session_')
    )
  }
      ...
      <div className="space-y-4">
        {nextAppointments.map((appointment, index) => (
          <div key={index} className="flex items-start gap-4">
            ...
            <div>
              {!isCoupleSession(appointment.type) &&
              appointment.mainUser &&
              appointment.partner ? (
                <div className="text-primary-cream">
                  <div className="font-medium">
                    {appointment.mainUser.firstName}{' '}
                    {appointment.mainUser.lastName}
                  </div>
                  <div className="font-medium text-primary-cream/80">
                    {appointment.partner.firstName}{' '}
                    {appointment.partner.lastName}
                  </div>
                </div>
              ) : (
                <div className="text-primary-cream">
                  ...
                  {appointment.user.isPartner && (
                    <span className="text-primary-cream/60 text-sm ml-2">
                      (partenaire)
                    </span>
                  )}
                </div>
              ...
              <p className="text-sm text-primary-coral mt-1">
                {getSessionType(appointment.type)}:{' '}
                {format(appointment.date, 'dd MMM yyyy à HH:mm', {
                  locale: fr,
                })}
              </p>
            </div>
