...
export function OffersPanel({ userId, partnerId, onOffersLoaded }: OffersPanelProps) {
      ...
      {offers.map((offer) => (
        ...
        >
          ...
          {offer.features && offer.features.length > 0 && (
            <div className="mt-4 space-y-2">
              {offer.features.map((feature, index) => (
                <div key={index} className="flex items-start gap-2 text-primary-cream/80">
          ...
          <div className="flex justify-between items-center mt-4 text-sm text-primary-cream/60 pt-4 border-t border-primary-cream/10">
            <span>Achetée le {new Date(offer.createdAt).toLocaleDateString('fr-FR')}</span>
            {offer.expiresAt && (
